...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    dJ_dVc = U.T.dot(delta)

    gradPred = dJ_dVc
    
    dJ_dUk = delta.reshape(-1,1).dot(Vc.reshape(-1,1).T)

    for k in range(K+1):
        grad[indices[k]] += dJ_dUk[k,:]
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    cwIndex = tokens[currentWord]
    cwVector = inputVectors[cwIndex:cwIndex+1]

    for targetWord in contextWords:
        twIndex = tokens[targetWord]
        c, gin, gout = word2vecCostAndGradient(cwVector, twIndex, outputVectors, dataset)
        
        gradIn[cwIndex, :] += gin.reshape(-1)
        gradOut += gout
        cost += c
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for contextWord in contextWords:
        ...
        c, gIn, gOut = word2vecCostAndGradient(input_vector, tokens[currentWord], outputVectors, dataset)
        cost += c
        gradIn[idx:idx+1, :] += gIn
        gradOut += gOut
    ### END YOUR CODE
